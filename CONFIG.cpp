...
//	dt= (switch_FEM==OFF) ? 1.0e-5: 5.0e-6; //0.0001;不安定要因！ 0.00001:推奨(Courant数考えて) //Cf. dt_for_FEM=0.000001/2;
	dt=1.e-6;//1.e-4;//1e-3;////5.e-3;//1.e-4;//5.e-3;//1e-5;
	dt_for_FEM=1.e-6;
	//FEMだと0.000001で止まる・・・
	current_step=2;
	current_time=0.0;
...
/*	ZU=distancebp*20;
	...
	//FRMcheck用	15/2/10
	ZU=0.1;//0.25;//0.10; //0.2
	ZD=-0.1;//0.25;//0.10; //0.2 				//液滴 -0.01 コイル:-0.15 るつぼ:-0.0002
	RU=0.1;//0.25;//0.10;//0.1;				//解析領域が円筒形となるときのその半径
...
//粒子配置用
	...
	distancebp=9.e-3/4.;//0.1;//10.e-3/5.;//	2.e-3;	//2.e-3;//10./3.*1.e-3;//0.005;///0.001/2;//0.005; //distancebp=0.0125;[mm]
	wlength=2;
	height=0.0;//0.005;    

//解析領域
	maxX=0.1;//0.25;//0.1;	//0.1/2;	//1
	minX=-0.1;//0.25;//-0.1;	//-0.1/2;
	maxY=0.1;//0.25;//0.1;	//0.1/2;	//0.4;
	minY=-0.1;//0.25;//-0.1;	//-0.1/2;	//-0.6; //-1.0
	maxZ=0.1;//0.25;//0.1;	//0.1/2;	//0.3;
	minZ=-0.1;//0.25;//-0.1;	//-0.1/2;	//-0.6;  //indexの関係上、Z方向には余裕をもつこと。
...
//メッシュ
	...
	FEMCGep=1.0e-5;			//1.3e-4PICCG 元々5.0e-6	//本プログラムでは現在不使用15/5/24
