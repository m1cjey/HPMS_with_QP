...
void q_QP_g(mpsconfig &CON,vector<mpselastic> &PART,vector<hyperelastic> &HYPER,vector<hyperelastic2> &HYPER1,int t, int h_num, int Nx, double V, double mi, double Dt, double nG[DIMENSION], double E0,double **F);
...
void q_variables_g(mpsconfig &CON, vector<mpselastic> &PART,vector<hyperelastic> &HYPER,vector<hyperelastic2> &HYPER1,double **F);
...
void calc_HYPER_QP_g(mpsconfig &CON,vector<mpselastic> &PART,vector<hyperelastic> &HYPER,vector<hyperelastic2> &HYPER1,int t,double **F)
{
	...
	if(t==1)
	{
		for(int i=0;i<h_num;i++)
		{
			...
			HYPER[i].E0=0.5/mi*(HYPER[i].p0[A_X]*HYPER[i].p0[A_X]+HYPER[i].p0[A_Y]*HYPER[i].p0[A_Y]+HYPER[i].p0[A_Z]*HYPER[i].p0[A_Z])+V*HYPER[i].W0+HYPER[i].Ef+mi*G*PART[i].q0[A_Z];
			E0_t+=0.5/mi*(HYPER[i].p0[A_X]*HYPER[i].p0[A_X]+HYPER[i].p0[A_Y]*HYPER[i].p0[A_Y]+HYPER[i].p0[A_Z]*HYPER[i].p0[A_Z])+V*HYPER[i].W0+HYPER[i].Ef+mi*G*PART[i].q0[A_Z];
		}
	...
	{
		for(int i=0;i<h_num;i++)	E0_t+=HYPER[i].E0;
	}
	cout<<"E0="<<E0_t<<endl;
	...
	////////////QPåvéZ///////////////		
	q_QP_g(CON,PART,HYPER,HYPER1,t,h_num,Nx,V,mi,Dt,nG,E0_t,F);
...
void q_QP_g(mpsconfig &CON,vector<mpselastic> &PART,vector<hyperelastic> &HYPER,vector<hyperelastic2> &HYPER1,int t, int h_num, int Nx, double V, double mi, double Dt, double nG[DIMENSION], double E0,double **F)
{
	...
	while(E_min>ep_min)
	{
		...
		{
			count++;
			q_variables_g(CON,PART,HYPER,HYPER1,F);
			...
			//	En+=0.5/mi*(hp[A_X]*hp[A_X]+hp[A_Y]*hp[A_Y]+hp[A_Z]*hp[A_Z])+V*W-mi*Ev+Efi+G*mi*PART[i].r[A_Z];
			...
			for(int i=0;i<h_num;i++)
			{
				...
				else
				{
					...
					{	
						p_p[A_X]+=F[A_X][i]*V;//density;
						p_p[A_Y]+=F[A_Y][i]*V;//density;
						p_p[A_Z]+=F[A_Z][i]*V;//density;
					}
					...
					//else
					//{
					//	PART[i].r[A_X]=HYPER[i].q_n[A_X]+Dt/ms*HYPER[i].half_p[A_X];
					//	PART[i].r[A_Y]=HYPER[i].q_n[A_Y]+Dt/ms*HYPER[i].half_p[A_Y];
					//	PART[i].r[A_Z]=HYPER[i].q_n[A_Z]+Dt/ms*HYPER[i].half_p[A_Z];	
